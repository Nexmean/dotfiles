...
local vertical_collapsed = function(self, max_columns, max_lines, layout_config)
  local initial_options = p_window.get_initial_window_options(self)
  local preview = initial_options.preview
  local results = initial_options.results
  local prompt = initial_options.prompt
  ...
  if self.previewer and max_lines >= layout_config.preview_cutoff then
    ...
    preview.height = resolve.resolve_height(vim.F.if_nil(layout_config.preview_height, 0.5))(self, max_columns, height)
  else
...
local vertical_collapsed_options = {
  width = { "How wide to make Telescope's entire layout", "See |resolver.resolve_width()|" },
  height = { "How tall to make Telescope's entire layout", "See |resolver.resolve_height()|" },
  scroll_speed = "The number of lines to scroll through the previewer",
  anchor = { "Which edge/corner to pin the picker to", "See |resolver.resolve_anchor_pos()|" },
...
function M.setup(default_layout_config)
  ...
  strategies["horizontal_collapsed"] = function(self, max_columns, max_lines, override_layout)
    return horizontal_collapsed(
      ...
      max_columns,
      max_lines,
      vim.tbl_deep_extend("keep", vim.F.if_nil(override_layout, {}), default_layout_config.horizontal_collapsed)
    )
  end
  strategies._configurations["vertical_collapsed"] = vertical_collapsed_options
  strategies["vertical_collapsed"] = function(self, max_columns, max_lines, override_layout)
    return vertical_collapsed(
      self,
      max_columns,
      max_lines,
      vim.tbl_deep_extend("keep", vim.F.if_nil(override_layout, {}), default_layout_config.vertical_collapsed)
    )
  end
end
